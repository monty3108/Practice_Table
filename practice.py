...
def practice_logic(mode, start, end):
    ...
    Now includes a mistake summary and waits for user input before exiting.
    """
    score = 0
    wrong = 0
    mistakes_summary = []
    ...
    while True:
        ...
        if user_ans == correct_ans:
            ...
            # Record the mistake details here
            mistake_record = f"{prompt.strip()} {correct_ans} (You answered: {user_ans})"
            mistakes_summary.append(mistake_record)

        if wrong >= 3:
            # Print the summary of mistakes before breaking
            print("\n   Too many mistakes! Here is a summary of what you missed:")
            for mistake in mistakes_summary:
                print(f"      - {mistake}")
            print("\n   Let's review these and try again.")
            break

    # <-- Changed the exit behavior here!
    print(f"\n   Final Score: {score}.")
    input("   Press Enter to return to the main menu...") 
    


def practice_logic_old(mode, start, end):
    """
    Unified logic for Tables, Squares, and Cubes.
    Tracks unique pairs/items to ensure each is answered correctly 3 times.
    """
